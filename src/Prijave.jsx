...
const newCount = prijave.filter(p => !p.status || p.status === "new").length;
const contactedCount = prijave.filter(p => p.status === "contacted").length;
const doneCount = prijave.filter(p => p.status === "done").length;
...
const deleteItem = (id) => {
 ...
 <div className="statsRow">
  ...
  <div className="statItem">🆕 Novih: {prijave.filter(p => !p.status || p.status === "new").length}</div>
  <div className="statItem">📞 Kontaktirani: {prijave.filter(p => p.status === "contacted").length}</div>
  <div className="statItem">✅ Završenih: {prijave.filter(p => p.status === "done").length}</div>
</div>
